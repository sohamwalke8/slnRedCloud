...
{
    public class GetAllOrganizationAdminQueryHandler : IRequestHandler<GetAllOrganizationAdminQuery, BaseResponse<List<AllOrganizationAdminVM>>>
    {
        ...
        public async Task<BaseResponse<List<AllOrganizationAdminVM>>> Handle(GetAllOrganizationAdminQuery request, CancellationToken cancellationToken)
        {
            var AllOrganizatinAdmin =  (await _asyncRepository.ListAllAsync().Include(x => x.ResellerAdmins)).Where(x => x.IsActive == true);

            var resellers = AllOrganizatinAdmin.ResellerAdmins;                        // List of ResellerAdmin associated with the OrganizationAdmin
            
            var AllOrganizatinAdminData = _mapper.Map<IEnumerable<AllOrganizationAdminVM>>(AllOrganizatinAdmin);
